...
- `im.pid` — планируемый id посылки при создании заказа
...
- `fm.pid` — фактический parcelId после консолидации (несколько `_id.o` → один `fm.pid`)
...
После консолидации система выбирает реальный канал доставки.
...
## Какие данные различаются между группами:

При создании заказа:

- `im.pid` — планируемый id посылки
- `im.ch` — канал доставки
- `im.rev.price` — стоимость логистики для пользователя

При поступлении заказа на склад:

- `fm.t` — время поступления

После консолидации заказов в посылки:

- `fm.pid` — фактический id после консолидации
- `fm.pw` — вес финальной посылки
- `fm.ch` — финальный логистический канал
- `fm.rev.price` — стоимость логистики для маркетплейса (тугрики)
